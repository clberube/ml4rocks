...
df.ix[(((df.LECO_S_pct >= 0.1)|(df.AR_Au_ppm >= 0.1))&(df.Code_Alteration.isin(codes[1]))), 'Code_Alteration'] = "SED-AL"

df.ix[(((df.LECO_S_pct >= 0.1)|(df.AR_Au_ppm >= 0.1))&(df.Code_Alteration.isin(codes[3]))), 'Code_Alteration'] = "MAF-AL"

df.ix[(((df.LECO_S_pct >= 0.1)|(df.AR_Au_ppm >= 0.1))&(df.Code_Alteration.isin(codes[5]))), 'Code_Alteration'] = "FEL-AL"

df.ix[(((df.LECO_S_pct < 0.1)&(df.AR_Au_ppm >= 0.1))&(df.Code_Alteration.isin(codes[1]))), 'Code_Alteration'] = "SED-UN"

df.ix[(((df.LECO_S_pct < 0.1)&(df.AR_Au_ppm >= 0.1))&(df.Code_Alteration.isin(codes[3]))), 'Code_Alteration'] = "MAF-UN"

df.ix[(((df.LECO_S_pct < 0.1)&(df.AR_Au_ppm >= 0.1))&(df.Code_Alteration.isin(codes[5]))), 'Code_Alteration'] = "FEL-UN"
...
codes = ['SED-UN', 'SED-AL', 'FEL-UN', 'FEL-AL', 'MAF-UN', 'MAF-AL']
...
#df.to_csv("MalarticPhysProp_Alterations")
...
#    seed = 7
    ...
    if li == 2:
        knn = SVC(C = 1, probability=True, class_weight='balanced', kernel='rbf', gamma=0.5)

    if li == 0:
        knn = SVC(C = 0.4, probability=True, class_weight='balanced', kernel='rbf', gamma=0.5)
...
#    knn = KNeighborsClassifier(n_neighbors=20)
    ...
    if li == 0:
        classed_results = pd.DataFrame(dict(label=knn.predict(X)))
        classed_results.set_index(dataset.index, inplace=True)
#        altered_seds = classed_results.index[(classed_results["label"] == "SED-AL")&(dataset["Code_Alteration"]=="SED-AL")]
        altered_seds = classed_results.index[(classed_results["label"] == "SED-AL")]

    if li == 1:
        classed_results = pd.DataFrame(dict(label=knn.predict(X)))
        classed_results.set_index(dataset.index, inplace=True)
        altered_fels = classed_results.index[(classed_results["label"] == "FEL-AL")]

    if li == 2:
        classed_results = pd.DataFrame(dict(label=knn.predict(X)))
        classed_results.set_index(dataset.index, inplace=True)
        altered_mafs = classed_results.index[(classed_results["label"] == "MAF-AL")]
...
#    den_fake = np.random.normal(np.median(X[:,0]), 1*np.std(X[:,0]), 10000)
    ...
    mark = ["s","s","o","o","^","^"]
    colors = ["#1f77b4", "white", "#ff7f0e", "white", "#2ca02c", "white"]
    ...
    titles = ["Meta-sedimentary rocks", "Felsic-interm. intrusive rocks", "Mafic dykes"]
    labels = ["Altered", "Unaltered", "Altered", "Unaltered", "Altered", "Unaltered"]
    ...
    for c, (name, group) in enumerate(initial_groups):
        ...
        ax[0,li].scatter(group.Rock_Den, group.Mag_Susc, s=20, color=colors[c], edgecolors="k", linewidth=0.2, alpha=0.7, marker=mark[c], label=labels[c])
    #    ax[0,0].set_title("Alterations SVM classification", fontsize=12)
        ax[0,li].legend(fontsize=10, loc=4,labelspacing=0.1,handletextpad=0,framealpha=1,markerscale=2)
    ax[0,li].grid(None)
...
#        a.axes.set_axisbelow("True")
    ...
    yi = np.linspace(ymin, ymax, 100)
    zi = ml.griddata(den_fake, mag_fake, Z, xi, yi, interp='linear')
    cf = ax[1,li].contourf(xi, yi, zi, np.arange(0.3, 1.01, .01), cmap=colormaps[li], alpha=0.3)
    cs = ax[1,li].contour(xi, yi, zi, np.arange(0.1, 1.1, levels[li]), cmap='Greys', linestyles='-', linewidths=1, alpha=1)
...
#    a.grid(False)
    ...
    ax[0,li].contourf(xi, yi, fake_zi, 6, cmap=cm, alpha=0.3,zorder=0)
...
#    ax[0,li].fill_between(xi, ymin, y2, facecolor='white', interpolate=True)
    ...
    headers = ["Rock type", "", "Precision", "Recall", "F1 score", "Support"]
    
    formatted_data = np.concatenate((np.array([["",""]]).T, np.array([labels]).T,arrays_table.T),axis=1)
...
for a in range(3):
    ...
    ax[1,a].set_xlabel("Grain density (g/cm$^3$)")
...
#    ax[a,1].set_ylim([-5, ymax])
    ...
    ax[a,0].set_xlim([2.65,2.95])
    ax[a,0].set_xlim([2.65,2.95])
    ax[a,1].set_xlim([2.6,2.9])
    ax[a,1].set_xlim([2.6,2.9])
...
fig.savefig("AlterationClassification.pdf", dpi=600, bbox_inches="tight")
