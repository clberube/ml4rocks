...
#                fake_Z[i] = int(5)         
    ...
    titles = ["Au cutoff: 0.01 ppm", "Au cutoff: 0.1 ppm", "Au cutoff: 1 ppm"]
...
#    a.grid(False)
    ...
    for c, (name, group) in enumerate(initial_groups):
        ...
        ax[0,li].scatter(group.Rock_Den, group.Mag_Susc, s=20, color=colors[c], edgecolors="k", linewidth=0.2, alpha=0.7, marker=mark[c], label=labels[c],zorder=2-c)
